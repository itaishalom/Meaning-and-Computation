...
def runOnSeeds(seedA, seedB, seedC):
    ...
    for key, value in arr:
        ...
        for entry in my_dict:
            ...
            toCheck = my_dict[entry]
            if type(toCheck) is sentence:
                    if key in toCheck.get_window():
                        keyCounter[toCheck.get_label()] = keyCounter[toCheck.get_label()]+1
            else:
                for wind in toCheck:
                    if key in wind.get_window():
                        keyCounter[wind.get_label()] = keyCounter[wind.get_label()] + 1

        keyInSenseA = keyCounter[seedA]
        keyInSenseB = keyCounter[seedB]
        keyInSenseC = keyCounter[seedC]
